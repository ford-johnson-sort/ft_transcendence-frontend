...
export class PongGameLogic {
	...
	#send() {
		...
		/**
		 * 호스트가 공의 위치를 관리하며, 모든 참가자들에게 동기화
		 */
		if (this.isHost) {
	...
	#recv(event) {
		...
		if (this.isGuest && data.ball_object) {
			...
			this.ball.position.x *= -1;	// 축 위치 반전
			this.ball.position.y *= -1;	// 축 위치 반전
			this.ball.velocity.x *= -1;	// 축 위치 반전
			this.ball.velocity.y *= -1;	// 축 위치 반전
